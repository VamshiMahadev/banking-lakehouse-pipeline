...
# MAGIC GROUP BY c.customer_id, c.first_name, c.last_name, c.email, c.phone_number, c.created_date;

# COMMAND ----------

# MAGIC %sql
# MAGIC -- DBTITLE 1, Daily Transaction Summary (Gold Delta Table)
# MAGIC CREATE OR REPLACE TABLE banking_catalog.gold.fact_daily_account_metrics
# MAGIC USING DELTA
# MAGIC AS
# MAGIC SELECT 
# MAGIC     account_id,
# MAGIC     DATE(txn_timestamp) AS txn_date,
# MAGIC     SUM(CASE WHEN UPPER(txn_type) IN ('DEPOSIT', 'CREDIT') THEN amount ELSE 0 END) AS total_deposits,
# MAGIC     SUM(CASE WHEN UPPER(txn_type) IN ('WITHDRAWAL', 'DEBIT') THEN amount ELSE 0 END) AS total_withdrawals,
...
# MAGIC %sql
# MAGIC -- DBTITLE 1, Performance Optimization with Z-Order
